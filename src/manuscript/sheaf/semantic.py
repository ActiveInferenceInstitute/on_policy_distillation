...
def _policy_comparison_restrictions(root: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "planners": sorted({str(row.get("planner")) for row in runs if row.get("planner")}),
        ...
        "vanilla_role": (data.get("summary") or {}).get("vanilla_role"),
...
def _pymdp_hash_restrictions(root: Path) -> dict[str, Any]:
    ...
    return {
        "planner_match": not summary or not stats or summary.get("planner") == stats.get("pymdp_planner"),
...
def semantic_gluing_issues(project_root: Path) -> list[str]:
    ...
    if summary and stats:
        if summary.get("planner") != stats.get("pymdp_planner"):
            issues.append(
                f"pymdp planner mismatch: summary={summary.get('planner')!r}, stats={stats.get('pymdp_planner')!r}"
            )
    ...
    if set(policy["planners"]) != {"sophisticated_inference", "vanilla"}:
        issues.append(f"policy comparison planner set invalid: {policy['planners']!r}")
    if policy["run_count"] < 2:
        issues.append(f"policy comparison run count too small: {policy['run_count']!r}")
    if policy["vanilla_role"] != "comparison_only":
        issues.append("vanilla planner is not marked comparison_only")
...
def build_semantic_gluing_certificate(project_root: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "restrictions": {
            ...
            "policy_comparison_planners": policy["planners"],
            ...
            "pymdp_planner_match": pymdp_hash["planner_match"],
