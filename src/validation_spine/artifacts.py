...
CORE_ARTIFACT_PRODUCERS: dict[str, str] = {
    ...
    "output/figures/sequential_shift_recovery.png": "generate_figures.py",
    "output/figures/sequential_shift_sensitivity.png": "generate_figures.py",
    ...
    "output/data/firstprinciples/sequential_shift.json": "generate_firstprinciples.py",
    "output/data/firstprinciples/sequential_shift_sensitivity.json": "generate_firstprinciples.py",
...
CORE_HASH_EXCLUSIONS: frozenset[str] = frozenset(
    {
...
def build_artifact_provenance(project_root: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "all_hashed": all(
            (record["exists"] and bool(record["sha256"])) or rel in CORE_HASH_EXCLUSIONS
        ...
        "cycle_hash_exclusions": sorted(CORE_HASH_EXCLUSIONS),
    }
...
def validate_artifact_provenance(project_root: Path) -> list[str]:
    ...
    for rel, live_record in (live.get("artifacts") or {}).items():
        ...
        if not isinstance(saved_record, dict):
            ...
            continue
        if rel not in CORE_HASH_EXCLUSIONS:
