...
def figure_tmaze_schematic(project_root: Path) -> Path:
    root = project_root.resolve()
    spec = TMazeSpec()
    config = yaml.safe_load((root / "pymdp.yaml").read_text(encoding="utf-8")) or {}
    ...
    if matrices.get("A_shapes") and matrices.get("B_shapes"):
        matrix_summary = f"A={matrices['A_shapes']}; B={matrices['B_shapes']}"
    with styled_figure(root, "tmaze_schematic") as (style, out):
        ...
        for key, label, color in node_rows:
            ...
            ax.add_patch(box)
            ax.text(center[0], center[1], label, ha="center", va="center", fontsize=8.2)
        for x, y, label in (
            (-1.62, -1.88, f"profile={config.get('profile', 'full_tmaze_sophisticated_inference')}"),
            (-1.62, -1.69, "3 obs modalities: location, outcome, cue"),
            (
                -1.62,
                -1.50,
            ...
            (-1.62, -1.31, f"SI agent policy_len={spec.policy_len}; search horizon={spec.planning_horizon}"),
            (-1.62, -1.12, matrix_summary),
        ):
            if label:
                ax.text(x, y, label, ha="left", va="center", fontsize=8, color=style.color("primary"))
        ax.text(
            1.62,
            -1.88,
        ...
        ax.set_title("pymdp full TMaze generative process")
        save_styled_figure(fig, out, style)
    return out
