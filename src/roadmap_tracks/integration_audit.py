...
from .integration_audit_builders import (
    ...
    build_hardcoded_variable_audit,
...
def write_integration_audit_artifacts(project_root: Path) -> dict[str, Path]:
    ...
    paths = {
        ...
        "hardcoded_variable_audit": _write_json(
            root / "output" / "reports" / "manuscript_hardcoded_variable_audit.json",
            build_hardcoded_variable_audit(root),
        ),
...
def validate_integration_audit_artifacts(project_root: Path) -> list[str]:
    ...
    if hardcoded.get("all_values_auto_injected") is not True or int(hardcoded.get("issue_count", 0) or 0) != 0:
        issues.append("manuscript_hardcoded_variable_audit.json records hard-coded generated values")
    live_hardcoded = build_hardcoded_variable_audit(root)
    saved_hardcoded_rows = [
        {key: row.get(key) for key in ("token", "guarded_literal", "literal_occurrences", "auto_injected")}
        for row in hardcoded.get("rows") or []
    ]
    live_hardcoded_rows = [
        {key: row.get(key) for key in ("token", "guarded_literal", "literal_occurrences", "auto_injected")}
        for row in live_hardcoded.get("rows") or []
    ]
    saved_hardcoded_summary = {
        key: hardcoded.get(key) for key in ("guarded_token_count", "issue_count", "all_values_auto_injected", "issues")
    }
    live_hardcoded_summary = {
        key: live_hardcoded.get(key)
        for key in ("guarded_token_count", "issue_count", "all_values_auto_injected", "issues")
    }
    if hardcoded and (saved_hardcoded_rows != live_hardcoded_rows or saved_hardcoded_summary != live_hardcoded_summary):
        issues.append("manuscript_hardcoded_variable_audit.json is stale relative to live manuscript source")
    figures = _load_json(root / "output" / "data" / "figure_source_map.json")
    figures_derived = _figure_source_rows_complete(root, figures)
    if figures.get("all_figures_mapped") is not True or figures.get("all_figures_mapped") != figures_derived:
        issues.append("figure_source_map.json has incomplete figure provenance")
    ...
    figure_hash_derived = _figure_hash_rows_complete(root, figure_hash)
    if (
