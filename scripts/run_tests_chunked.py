...
def main(argv: list[str] | None = None) -> int:
    ...
    for i, chunk in enumerate(chunks, 1):
        ...
        print(f"chunk {i}/{len(chunks)} [{marker}]: {summary}")
        if proc.returncode != 0:
            bad_chunks.append(i)

    print(f"TOTAL: {total_passed} passed, {total_failed} failed, {total_skipped} skipped across {len(chunks)} chunks")
    if bad_chunks:
        print(f"FAIL: chunk(s) {bad_chunks} did not exit clean")
        return 1
    return 0
