...
| `AI-SCOPE-ROWS-1` | Scope boundary | Make scope classification row-level for current, future, empirical, and blocked-language contexts. *Run-6 audit: PARTIAL — 16 row-level section rows but a single `toy_or_future` class via substring match over top-level sections only; flag-flip negative control exists, wording-mutation control does not* | `output/reports/scope_boundary_audit.json` | `all` current-result rows classified toy-only | Empirical wording appears outside future/blocked context |
