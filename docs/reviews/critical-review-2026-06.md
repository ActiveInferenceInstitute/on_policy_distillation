...
| Do not perform DOI, Zenodo, public archive, or GitHub release publication | **LANDED (2026-08-02)** | Publication proceeded as the v1.0.2 release: `manuscript/config.yaml` now carries populated `doi` / `version_doi` fields; `CITATION.cff`, `.zenodo.json`, `codemeta.json`, and `STANDALONE.md` carry the version record `10.5281/zenodo.20749817`; the release tags `v1.0.0`–`v1.0.2` exist and both DOIs resolve. `src/roadmap_tracks/scholarship/sources_review.py` still marks FAIR as context only |
