...
| Qwen/Thinking Machines numbers need primary-source anchoring; TM must not be load-bearing | **LANDED** (table/figure pin **DEFERRED**) | Discussion "Empirical evidence (literature-reported)": "We did **not** measure any of the following ourselves; the table values … are from the Qwen3 technical report as relayed and discussed by Thinking Machines … reproduced here only as external context." Pinning the exact Qwen table/figure number requires re-verification against the primary PDF and is tracked in `TODO.md` (do not invent a table number without re-opening the source) |
...
| External OPD-vs-RL table: pin exact Qwen table/figure | **DEFERRED** | Requires re-opening the primary Qwen3 report; the manuscript already brackets all values as literature-reported. Do **not** insert a table number without re-verification against the primary source |
