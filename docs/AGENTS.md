...
`working/active_inference_on_policy_distillation/docs/` is documentation only. Keep business
...
- Update project tests under `../tests/`; update sibling-template contract tests
  only when the public template render contract changes.
...
- Keep pymdp simulation claims aligned with what `src/simulation/si_runner.py`
  actually logs: the canonical full TMaze `sophisticated_inference` planner via
  `simulate_si_tmaze.py`; vanilla rows are validation-comparison artifacts only.
