...
* C1/C2 — SI rollout: planner advertised == planner run; summary records a measured rollout.
...
def _write_summary(root: Path, *, steps: int, planner: str) -> None:
...
def _write_pymdp_cfg(root: Path, planner: str) -> None:
    (root / "pymdp.yaml").write_text(
        "\n".join(
            [
                "profile: full_tmaze_sophisticated_inference",
                f"planner: {planner}",
...
def test_binding_flags_unmeasured_si_summary(tmp_path: Path) -> None:
    ...
    _write_summary(tmp_path, steps=0, planner="sophisticated_inference")
    _write_pymdp_cfg(tmp_path, "sophisticated_inference")
...
def test_binding_passes_measured_consistent_si_summary(tmp_path: Path) -> None:
    _write_summary(tmp_path, steps=2, planner="sophisticated_inference")
    _write_pymdp_cfg(tmp_path, "sophisticated_inference")
    assert verify_claim_bindings(tmp_path) == []


def test_binding_flags_planner_mismatch(tmp_path: Path) -> None:
    """Manuscript advertises the config planner; recorded rollout planner must match."""
    _write_summary(tmp_path, steps=2, planner="sophisticated_inference")
    _write_pymdp_cfg(tmp_path, "unexpected_planner")
    violations = verify_claim_bindings(tmp_path)
    assert any("planner mismatch" in v for v in violations), violations
