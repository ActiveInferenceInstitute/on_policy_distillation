...
def test_semantic_certificate_covers_tracks_symbols_and_variables(project_root: Path) -> None:
    ...
    assert cert["shared_symbols"]["si_tmaze"]["q_pi"] == "PolicyPosterior"
...
def test_typed_claim_evidence_supports_structured_predicates(tmp_path: Path) -> None:
    ...
    artifact.write_text(
        json.dumps(
            {
                "planners": ["sophisticated_inference", "vanilla"],
    ...
    ledger.write_text(
        "\n".join(
            [
                "claims:",
                "  - id: planners",
                "    statement: planner set",
                "    path: output/data/predicate_payload.json",
                "    tracks: [pymdp]",
                "    evidence:",
                "      field: planners",
                "      set_equals: [sophisticated_inference, vanilla]",
                "  - id: run_count",
                "    statement: two runs",
                "    path: output/data/predicate_payload.json",
                "    tracks: [pymdp]",
