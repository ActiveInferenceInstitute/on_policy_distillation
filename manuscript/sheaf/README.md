...
Appendix proof size is derived from the manifest. It binds the full proof row, including the generated `layers` report and optional-type `animation`, so the appendix exercises every registered fragment renderer.
